...
|---|---|---|
| 0.13.0 | 2026-08-17 | Novo composite **`PageHeader`** — a faixa de módulo (Banner de Conteúdo) promovida do `PageHeader` do Governança BI, que 14 telas usavam como componente local. API: `title`/`description`/`eyebrow`/`icon`/`info`/`badge`/`actions`/`stats`/`compact`/`as`. `PatternPanelHero` virou adaptador `@deprecated` sobre ele (uma implementação só). **`PageHero` alinhado** com a versão que o fips-suprimentos já rodava: fundo `--fips-banner-page-bg`, trilhos de junção, prop `compact` e **padding embutido** (breaking: filho com `px/py` próprio dobra o respiro). Doc do Banner (§ 03/§ 04) passou a renderizar o componente real em vez de réplica com hex fixo, e a função de copiar saiu das páginas Banner, Raios, Sombras, Textarea e Tooltip. Os modais de tutorial (tour de boas-vindas e balão do botão Tutorial) foram alinhados ao padrão de modal do DS — header hero, Elemento Caixa e `Button` governado, via o novo `TutorialHero` compartilhado. Skill do menu **Ações** reescrita (presets, API, geometria da órbita, teclado, tokens de vidro, não-faça) e `RowActionsMenu` passou a repassar `ariaLabel` (como já tinha saído de Tabs/Table/Drawer/Header/Sidebar/Hero). Fixes de token: tile âmbar volta a 10%/19% (o `#FDC24E18`/`30` do banner é alfa hex, estava lido como 18%/30%) e trilho da faixa de conteúdo ganha opacidade própria (0.06 claro), separada da faixa de página (0.12) |
...
| 0.14.1 | 2026-08-18 | **Cabeçalho de tabela centralizado em todas as superfícies.** `TableHead` (que já vinha `text-center`) entra na lista de `governance/no-visual-overrides` — `text-left` num `th` agora é erro de lint, e não decisão de tela: o alinhamento já tinha ido e voltado duas vezes. Corrigidos os `<th>` que escapavam: preview do `ExportPreviewModal`, `ExportModal` legado, HTML de impressão e de PDF (`exportData.ts`), checkbox do Data Listing e as tabelas de referência das doc pages. O `td` continua respeitando `align` |
| 0.14.0 | 2026-08-18 | **Tokens com um dono só**: `src/tokens/theme.ts` vira a fonte da verdade e `src/styles/tokens.generated.css` passa a ser gerado por `npm run tokens:build` — antes cada cor existia duas vezes (TS + `@theme`), sincronizadas à mão. `npm run tokens:check` roda antes dos dois builds e no CI. Nova regra **`governance/no-raw-color`**: hex/`rgba()`/`hsl()` é **erro** na biblioteca publicada (hoje em zero) e warn no resto, com gate `npm run lint:colors`. Mortos os seletores de dark que casavam string de `style` inline (`[style*="#002A68"]`, `background:#333B41 !important` no hero) — dois não casavam mais nada e o terceiro sobrescrevia heros que já usavam token; no lugar entrou `--color-junction-stroke` nos 152 traços de trilho. `semanticColors`/`darkSemanticColors` deixaram de divergir do CSS (`border`, `surfaceMuted`, `foregroundMuted`, `sidebar`, `sidebarDeep`, `warning` dark) |
| 0.13.0 | 2026-08-17 | `PageHeader` (Banner de Conteúdo) promovido do Governança BI para a library, com fundo/sombra/raio vindos de `--fips-banner-*` — claro e escuro já resolvidos. `PatternPanelHero` vira adaptador `@deprecated` sobre ele |
| 0.12.4 | 2026-08-17 | Barra de filtros do Dashboard (`/docs/patterns/dashboard`) migrada para a **toolbar canônica** do Data Listing: faixa única (era 2 linhas empilhadas) com **Filtros** (outline azul + badge, abre **Drawer** novo com os 6 campos), **Busca** por código/solicitante, chips com **recolhimento progressivo** e o par **Excel/PDF** (`ExportButtons`) no lugar do botão "Relatório" com label. `xlsx` entra por import dinâmico — sem isso o chunk da página ia de 435 kB para 719 kB. Na toolbar do Data Listing: removido spacer `flex:1` que competia com a busca e criava vão vazio depois do Período, e adicionado `minWidth:180` na busca pra ela não ser espremida até cortar o placeholder |

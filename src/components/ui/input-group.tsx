...
const inputGroupAddonVariants = cva(
  'flex h-auto cursor-text select-none items-center justify-center gap-2 py-1.5 text-sm font-medium text-[var(--color-fg-muted)] group-data-[disabled=true]/input-group:opacity-50 [&>svg:not([class*="size-"])]:size-4',
  {
    variants: {
      align: {
        'inline-start': 'order-first pl-4 has-[>button]:ml-[-0.45rem]',
        'inline-end': 'order-last pr-4 has-[>button]:mr-[-0.4rem]',
        'block-start': 'order-first w-full justify-start px-4 pt-3 group-has-[>input]/input-group:pt-2.5',
        'block-end': 'order-last w-full justify-start px-4 pb-3 group-has-[>input]/input-group:pb-2.5',
      },
    },
    defaultVariants: {
      align: 'inline-start',
...
function InputGroupText({ className, ...props }: React.ComponentProps<'span'>) {
  return (
    <span
      className={cn(
        "flex items-center gap-2 text-sm text-[var(--color-fg-muted)] [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none",
        className,
      )}
      {...props}
    />
